...
def run_maigret(username: str) -> list:
    ...
    for line in output.splitlines():
        ...
        site = line[:url_match.start()].strip()
        site = re.sub(r'^\[\+\]\s*', '', site).strip()
        if site and url:
            results.append({"site": site, "url": url, "found": True})
    return results

def run_whatsmyname(username: str) -> list:
    output = run_cmd(["whatsmyname", "-u", username])
    results = []
    for line in output.splitlines():
        line = line.strip()
        ...
        try:
            ...
            if isinstance(data, dict):
                site = data.get("site", data.get("username", ""))
                url = data.get("uri_check", data.get("url", ""))
                if site and url.startswith("http"):
                    results.append({"site": site, "url": url, "found": True})
...
async def query_whois(domain: str) -> dict:
    try:
        ...
        result = await asyncio.to_thread(whois_lib.whois, domain)
        if result:
            text = str(result)
...
async def search_username(q: str):
    results_maigret = await asyncio.to_thread(run_maigret, q)
    results_wn = await asyncio.to_thread(run_whatsmyname, q)
    combined = []
    seen = set()
...
async def search_username_route(q: str):
    return await search_username(q)
...
async def search_phone(q: str):
    data = await asyncio.to_thread(run_phoneinfoga, q)
